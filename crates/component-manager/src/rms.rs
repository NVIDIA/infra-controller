...
use model::rack::{
    FirmwareUpgradeDeviceInfo, FirmwareUpgradeDeviceStatus, FirmwareUpgradeJob, NvosUpdateJob,
    NvosUpdateSwitchStatus,
};
...
fn rack_firmware_job_from_response(
        ...
        && response.jobs.is_empty()
    {
        ...
        return Err(ComponentManagerError::Internal(message));
    }
...
fn finish_rack_firmware_job(mut job: FirmwareUpgradeJob) -> FirmwareUpgradeJob {
    ...
    job.completed_at = if total > 0 && terminal == total {
        Some(chrono::Utc::now())
    } else {
        None
    };
