...
fn admit_v6_packet(
    ...
    record_v6_request_received(packet);
...
fn validate_v6_source_port(packet: &[u8], source: &SocketAddrV6) -> Result<(), DhcpError> {
    ...
    if source.port() != expected_port {
        return Err(DhcpError::InvalidInput(format!(
            "DHCPv6 {sender} packet originated from UDP port {}, expected {expected_port}",
            source.port()
        )));
    }
    Ok(())
    ...
    loop {
        tokio::select! {
            ...
            result = context.socket.recv_from(&mut buffer) => {
                ...
                let Some(permit) = admit_v6_packet(&buffer[..length], &rate_limiter) else {
                    continue;
                };
...
mod test {
    ...
    fn rate_limited_v6_packet_is_counted_at_ingress() {
        ...
        assert!(admit_v6_packet(&packet, &rate_limiter).is_none());
        assert_eq!(
