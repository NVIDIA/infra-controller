...
- The NVLink Manager certificate monitor cannot automatically rotate an NVLink switch certificate that has already expired because it cannot establish the TLS connection needed to inspect the certificate. It logs the connection error without starting rotation. Track the fix in [GitHub issue #4568](https://github.com/NVIDIA/infra-controller/issues/4568).
