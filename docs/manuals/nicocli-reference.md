...
The TUI auto-discovers any file matching `~/.nico/config*.yaml`. Use one file per environment:

```
~/.nico/config.yaml                # default
~/.nico/config.dev.yaml            # local kind dev
~/.nico/config.az51-dev2.yaml      # shared dev cluster
...
| `api.name` | API path segment between org and resource. Defaults to `nico` in fresh installs; production-like deployments often set this to `carbide` or another deployment-specific value. |

`api.name` is the most common source of misconfiguration. If every command returns:

```
HTTP/1.1 404 Not Found
{"source":"carbide","message":"The requested path could not be found"}
```

your `api.name` does not match the deployment. Find the deployment's expected value (in the API server's running config; for carbide-rest deployments it's the `carbide-rest-api-config` configmap, key `api.name`) and set it in your config.
...
auth:
  ...
  # Shell script that prints a bearer token on stdout. Recommended for
  # deployments fronted by auth providers nicocli does not natively support
  # (e.g. SSA).
...
### Token command (recommended for SSA and external auth)

When your auth provider's token semantics don't match nicocli's built-in OIDC grants -- for example, SSA-fronted clusters that require a non-default scope such as `scope=carbide` -- write a small script that prints a bearer token to stdout and reference it from config:
...
```
nicocli --config ~/.nico/config.az51-dev2.yaml \
...
Example for an SSA-fronted cluster using HashiCorp Vault for client credentials:

```bash
#!/usr/bin/env bash
set -euo pipefail

# Vault session must already be valid; this script does not perform interactive login.
creds="$(vault read -format=json services/<your-path>/issue/creds)"
client_id="$(printf '%s' "$creds"     | jq -r '.data.client_id')"
client_secret="$(printf '%s' "$creds" | jq -r '.data.secret')"

curl -sS -u "${client_id}:${client_secret}" \
     -d grant_type=client_credentials \
     -d scope=carbide \
     https://<your-ssa-host>/token \
  | jq -r '.access_token'
```
...
> **OIDC scope is hardcoded**: the built-in password and client-credentials grants both send `scope=openid` -- this is not configurable. If your IdP requires a different scope (a common gotcha for non-Keycloak deployments), use `auth.token_command` instead.
...
time=... msg="API request: GET http://localhost:8388/v2/org/my-org/carbide/tenant/current"
...
nicocli substitutes the API name segment in every URL before sending. The OpenAPI spec uses `nico` as the placeholder; the deployment may use any value (`carbide` is common on production-like dev clusters). nicocli rewrites `/v2/org/<org>/nico/...` to `/v2/org/<org>/<api.name>/...` transparently. `--debug` shows the rewritten URL, which is what you should see in the API server's access log.
