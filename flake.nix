    ...
    flake-utils.lib.eachSystem [
      ...
      let
        ...
        aarch64ServerBinaries =
          let
            mk = pname: mkCrossCrateBinary { inherit pname; meta = carbideMeta; };
          in
          {
        ...
        # ====================================================================
        restApiVendorHash = "sha256-1fDSqFVw56tvrMpnrSMDUF8ryL3Mwb5R8r7B5nkgXf0=";
        ...
        serviceSpecs = import ./nix/services { inherit mftFor nsmStaticFiles; };
        ...
        containers =
          let
            ...
            # not a service, and ships no image.
      ...
      in
      {
        ...
        # bootloaders, and the vendor packages from nix/third-party/. No name
        # overlap — every attribute is unique. From an x86 dev machine
        # `nix build .#forge-dpu-agent` cross-compiles with no aarch64 hardware
