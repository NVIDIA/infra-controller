...
| `bmc_proxy` | unset | Routes nico-api's own BMC Redfish traffic through nico-bmc-proxy (`enabled`, `url`, SPIFFE cert paths), including established-endpoint credentialed exploration. Credential setup and rotation, session minting, and exploration's anonymous vendor probes always stay direct. |
